...
pub fn load(client: &FarmClient, config: &Config, data: &str, remove_mode: bool) {
    ...
    for val in funds {
        ...
        if !remove_mode {
            ...
            client
                .remove_fund(config.keypair.as_ref(), &json_fund.name.to_string())
                .unwrap();
            continue;
        }
        let (index, counter) = if let Ok(fund) = client.get_fund(&json_fund.name.to_string()) {
