...
impl<'a, 'b> FundInfo<'a, 'b> {
    ...
    pub const LIQUIDATION_ID_INDEX: usize = 20;
    pub const LIQUIDATION_TOKEN_AMOUNT_INDEX: usize = 21;
    ...
    pub fn init(&mut self, refdb_name: &ArrayString64) -> ProgramResult {
        ...
        self.init_refdb_field(
            FundInfo::LIQUIDATION_ID_INDEX,
            "LiquidationId",
            Reference::U64 { data: 0 },
        )?;
        self.init_refdb_field(
            FundInfo::LIQUIDATION_TOKEN_AMOUNT_INDEX,
    ...
    pub fn set_liquidation_id(&mut self, liquidation_id: u64) -> ProgramResult {
        RefDB::update_at(
            &mut self.data,
            FundInfo::LIQUIDATION_ID_INDEX,
            &Reference::U64 {
    ...
    pub fn set_liquidation_token_amount(&mut self, liquidation_token_amount: u64) -> ProgramResult {
        RefDB::update_at(
            &mut self.data,
            FundInfo::LIQUIDATION_TOKEN_AMOUNT_INDEX,
            &Reference::U64 {
                data: liquidation_token_amount,
            },
        )
        .map(|_| ())
    }
    ...
    pub fn get_liquidation_id(&self) -> Result<u64, ProgramError> {
        if let Some(rec) = RefDB::read_at(&self.data, FundInfo::LIQUIDATION_ID_INDEX)? {
            if let Reference::U64 { data } = rec.reference {
                return Ok(data);
            }
        }
    ...
    pub fn get_liquidation_token_amount(&self) -> Result<u64, ProgramError> {
        if let Some(rec) = RefDB::read_at(&self.data, FundInfo::LIQUIDATION_TOKEN_AMOUNT_INDEX)? {
            if let Reference::U64 { data } = rec.reference {
                return Ok(data);
            }
        }
